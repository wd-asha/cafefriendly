...
@section('content')
            ...
            @foreach($cartItems as $cartItem)
                ...
                <form action="{{route('cart.update')}}" method="POST" name="form-update">
                    @csrf
                    <div class="list-item">
                        <a href="" class="list-item-img" style="margin-right: 1rem">
                            <img src="{{ $cartItem->options->image }}" alt="">
                        </a>
                        <div class="list-item-desc">
                            <a href="" class="desc-title">{{ $cartItem->name }} ({{ $cartItem->qty }})
                            </a>
                            <div class="desc-subtitle">{!! $cartItem->options->about !!}</div><br>
                            @if($cartItem->weight != 0)
                            <div class="desc-subtitle">{{ $cartItem->weight }} гр.</div>
                            @endif
                            <div class="desc-price" style="color: #ff8000; font-weight: 600;">{{ $cartItem->price(0, ',', ' ' ) }} ₽</div>
                        </div>
